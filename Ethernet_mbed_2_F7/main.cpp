...
#define SERVER_PORT_RX 10000
#define OWN_PORT_RX    10001

//SocketAddress udp_server_address_RX("192.168.0.133", 10000);
//SocketAddress udp_server_address_TX("192.168.0.133", 10001);

Endpoint endpoint_server_RX;
Endpoint endpoint_server_TX;
...
//    socket.set_blocking(false, 0);
    // needed for the external ethernet lib
    //udp_server.set_address(SERVER_ADDRESS, SERVER_PORT);


    // this is for mbed OS
    // ACHTUNG, entscheidend ist die Reihenfolge !!!
    // zuerst socket öffnen, dann bind und set_blocking machen !!!
    //socketRX.open(&eth);
    //socketRX.bind(10001);
    //socketRX.set_blocking(false);
...
//    }
    ...
    // ACHTUNG: hier server_address NICHT als Pointer übergeben
    // sendBytesCount = socketTX.sendto(udp_server_address_RX, (char*)&messageToSend, sizeof(messageToSend)); //(char *)
...
//    fastParameterValues[RECEIVED_BYTES_COUNT] = (float)socketRX.receiveFrom(endpoint_server_TX, (char *)&receivedParameters, sizeof(receivedParameters));
    ...
    fastParameterValues[RECEIVED_BYTES_COUNT] = (float)socketRX.receiveFrom(endpoint_server_TX, (char *)&messageReceived, sizeof(messageReceived));
...
float generateSetPoint(float time)
{
    ...
    switch ((int)(controlledParameterValues[SP_GEN1_NUMBER] + 0.5f))
    {
        ...
        case 3:
            // generates a sinus
            setPoint = controlledParameterValues[SP_GEN1_SIN_AMPLITUDE] * (float)sin(controlledParameterValues[SP_GEN1_SIN_OMEGA] * time) + controlledParameterValues[SP_GEN1_SIN_OFFSET];
            break;
